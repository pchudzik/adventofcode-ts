...
import { part1IsValidPassword, part2IsValidPassword, passwordFactory } from './04';

function main() {
  const min = 284639
  const max = 748759
  const passwordGenerator1 = passwordFactory(min, max, part1IsValidPassword)
  const passwordGenerator2 = passwordFactory(min, max, part2IsValidPassword)
  console.log('1: ', Array.from(passwordGenerator1).length);
  console.log('2: ', Array.from(passwordGenerator2).length);
}
