...
can detect any asteroid to which it has direct line of sight - that is, there cannot be another asteroid exactly between
them. This line of sight can be at any angle, not just lines aligned to the grid or diagonally. The best location is the
...
Here is an asteroid (#) and some examples of the ways its line of sight might be blocked. If there were another asteroid
at the location of a capital letter, the locations marked with the corresponding lowercase letter would be blocked and
...
function main() {
  ...
  const asteroid = part2(puzzle)[199];
  console.log('part 2: ', asteroid.x * 100 + asteroid.y);
}
