...
function main() {
  const puzzle = fs
    .readFileSync('06.txt', 'utf-8')
    .split('\n')
    .filter(p => p && p.trim() !== '');

  const planetSystem = parsePlanets(puzzle);

  console.log('1: ', planetSystem.countAllOrbits());
  console.log('2: ', planetSystem.countOrbitJumps('YOU', 'SAN'));
}
